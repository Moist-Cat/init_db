...
def calculate_relevancy(movie):
    ...
    vote_factor = np.log(
        votes + 1
    )  # Use log scale for votes (larger impact on larger counts)
...
def sort_by_relevancy(movies):
    ...
    sorted_movies = sorted(
        movies_with_scores, key=lambda x: x["relevancy_score"], reverse=True
    )
...
def display_top_movies(movies, num_movies=10):
    ...
    for movie in top_movies:
        ...
        st.write(
            f"Rating: {movie['rating']} | Votes: {movie['votes']} | Relevancy Score: {movie['relevancy_score']}"
        )
...
def calculate_actor_metrics(movies):
    ...
    for movie in movies:
        ...
        for actor in actors:
            ...
            actor_metrics[actor]["performance_score"] = score_actor_performance(
                actor_metrics[actor]
            )
...
def score_actor_performance(metrics):
    ...
    score = (
        (metrics["num_movies"] * 0.2)
        + (metrics["avg_rating"] * 0.5)
        + (metrics["good_movies"] * 1.0)
    )
    return score
...
def plot_actor_performance(actor_metrics):
    ...
    performance_score = [
        metrics["performance_score"] for metrics in actor_metrics.values()
    ]
...
def calculate_actor_metrics_for_top_actors(actor_metrics, movies, top_n=1):
    ...
    sorted_actors = sorted(
        actor_metrics.items(), key=lambda x: x[1]["performance_score"], reverse=True
    )
    ...
    top_actor_movies = [
        movie
        for movie in movies
        if any(actor in top_actors for actor in movie.get("actors", []))
    ]
...
def calculate_missing_genre_providers(movies_data):
    ...
    genre_provider_counts = genre_df.groupby("Genre")["Provider"].apply(
        lambda x: list(set(x))
    )

    # Create a DataFrame to count movies per provider per genre
    provider_count = (
        genre_df.groupby(["Genre", "Provider"]).size().unstack(fill_value=0)
    )
...
def calculate_coverage_rate_by_genre(movies_data, genre_df):
    ...
    for genre in all_genres:
        ...
        for provider in providers:
            ...
            covered_movies = genre_movies[
                genre_movies["Provider"].apply(lambda x: provider in x)
            ]
            coverage_rate = (
                len(covered_movies) / total_movies_in_genre * 100
                if total_movies_in_genre > 0
                else 0
            )
...
def show_technical_metrics_for_genre(
    genre, movies_data, genre_provider_counts, genre_df, provider_coverage
):
    ...
    genre_movies = [
        ...
        for movie in movies_data
        if genre
        in [g for g in movie.get("tags", []) if g not in ["max", "amazon", "netflix"]]
    ]

    # Calculate the mean rating for the genre
    genre_ratings = [movie.get("rating", 0) for movie in genre_movies]
    genre_votes = [movie.get("votes", 0) for movie in genre_movies]
    mean_rating = sum(genre_ratings) / len(genre_ratings) if genre_ratings else 0
    mean_votes = sum(genre_votes) / len(genre_votes) if genre_votes else 0

    # Calculate the percent of movies with at least one provider
    genre_providers = (
        genre_df[genre_df["Genre"] == genre]["Provider"].apply(lambda x: bool(x)).sum()
    )
    percent_with_provider = (
        (genre_providers / len(genre_movies)) * 100 if genre_movies else 0
    )
    ...
    coverage_rate = (
        provider_coverage.get("max", {}).get(genre, 0),
        provider_coverage.get("amazon", {}).get(genre, 0),
        provider_coverage.get("netflix", {}).get(genre, 0),
    )

    # Display metrics in the sidebar
    st.sidebar.write(f"### {genre} Technical Metrics")
    st.sidebar.write(f"**Average Rating**: {mean_rating:.2f}")
    st.sidebar.write(f"**Average Votes**: {mean_votes:.2f}")
    st.sidebar.write(
        f"**Percent of Movies with Providers**: {percent_with_provider:.2f}%"
    )
...
def actors(sorted_movies):
    actor_stats_option = st.sidebar.selectbox(
        "Show Actor Metrics", ["Top 1 Actor", "Top 3 Actors"]
    )
    ...
    top_actor_stats = calculate_actor_metrics_for_top_actors(
        actor_metrics, sorted_movies, top_n=top_n
    )
    ...
    st.sidebar.write(
        f"**25th Percentile Rating**: {top_actor_stats['perc_rating_25']:.2f}"
    )
    st.sidebar.write(
        f"**50th Percentile Rating**: {top_actor_stats['perc_rating_50']:.2f}"
    )
    st.sidebar.write(
        f"**75th Percentile Rating**: {top_actor_stats['perc_rating_75']:.2f}"
    )
    st.sidebar.write(
        f"**25th Percentile Votes**: {top_actor_stats['perc_votes_25']:.0f}"
    )
    st.sidebar.write(
        f"**50th Percentile Votes**: {top_actor_stats['perc_votes_50']:.0f}"
    )
    st.sidebar.write(
        f"**75th Percentile Votes**: {top_actor_stats['perc_votes_75']:.0f}"
    )
...
def infer_distribution(data):
    """
    This function infers the distribution type and parameters for a given data sample.

    Args:
    - data: a pandas Series (numeric data)
    ...
    best_fit = None
    best_params = None
    best_p_value = -np.inf  # We aim to maximize the p-value
    ...
    for dist in distributions:
        ...
        # Check if the current distribution gives a better p-value
        if p_value > best_p_value:
            best_fit = dist
            best_params = params
            best_p_value = p_value

    return best_fit.name, best_params, best_p_value

def show_hip_test():
    titles = "shortened_title.basics.tsv"
    #titles = "title.basics.tsv"
    ratings = "shortened_title.ratings.tsv"
    #ratings = "title.ratings.tsv"

    titles_df = pd.read_csv(titles, sep='\t', low_memory=False)
    ratings_df = pd.read_csv(ratings, sep='\t', low_memory=False)

    # Data cleaning (drop null values, handle missing or inappropriate data)
    titles_df = titles_df.dropna(subset=['startYear', 'genres'])
    ratings_df = ratings_df.dropna(subset=['averageRating', 'numVotes'])

    # Merge the datasets on titleId to have both ratings and basic info in one dataframe
    movies_df = pd.merge(titles_df[['tconst', 'startYear', 'genres']], ratings_df[['tconst', 'averageRating', 'numVotes']], on='tconst')

    st.subheader("Misc hypothesis tests")
    best_fit_name, best_params, best_p_value = infer_distribution(movies_df['averageRating'])
    st.write(f"Best Fit Distribution: {best_fit_name}")
    st.write(f"Parameters: {best_params}")
    st.write(f"P-value: {best_p_value}")


    st.subheader("Testing Hypothesis: Are there significant relationships?")
    st.write("H0: There is no significant relationship between numVotes and averageRating")
    st.write("H1: There is a significant relationship between numVotes and averageRating")

    # T-test to check the significance of the correlation
    t_stat, p_val = stats.pearsonr(movies_df['numVotes'], movies_df['averageRating'])
    st.write(f"T-statistic: {t_stat}, P-value: {p_val}")

    # If p-value < 0.05, reject the null hypothesis (significant relationship)
    if p_val < 0.05:
        st.write("Reject H0: There is a significant relationship between numVotes and averageRating")
    else:
        st.write("Fail to reject H0: There is no significant relationship between numVotes and averageRating")
...
def run_streamlit_app():
    ...
    selected_directory = st.sidebar.selectbox(
        "Select metadata directory", metadata_dirs
    )
    ...
    provider_count, genre_df, genre_provider_counts = calculate_missing_genre_providers(
        movies_data
    )
    ...
    show_technical_metrics_for_genre(
        selected_genre, movies_data, genre_provider_counts, genre_df, provider_coverage
    )
